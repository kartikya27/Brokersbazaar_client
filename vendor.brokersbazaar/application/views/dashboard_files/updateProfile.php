...
It's quick and easy.
        ...
        <div class="col-xl-8 col-lg-8 col-md-8 col-12 ">
          ...
          foreach($vendorConnection as $vendor){ 
...
<!--        <label class="form-control-label px-3">-->
                ...
                <form class="form-card" action="<?php echo base_url();?>Vendor/details" method="post">
                    <div class="row justify-content-between text-left">
                             ...
                             <input type="hidden" name="license" class="md-input form-control" id="emailid" placeholder="email id"  value="<?php echo $vendor->email;?>"readonly>
                ...
                <input type="text" name="NSE_clr_no" class="md-input form-control" disabled id="NSE_clr_no" placeholder="NSE TM Clearing No">
                </p>
                </div>
                ...
                <input type="text" name="gst" class="md-input form-control" placeholder="GST No." disabled id="gst_in"> 
                    ...
                    <div class="form-group col-sm-12"> <button type="button" name="submitVendor" class="btn-block btn-primary btn_update">Update Details</button> </div>
...
<script>
  ...
  // Get the output text
  var text = document.getElementById("NSE_clr_no");

  // If the checkbox is checked, display the output text
  if (checkBox.checked == true){
    document.getElementById("NSE_clr_no").disabled = false ;
  } else {
    document.getElementById("NSE_clr_no").disabled = true;
  }
}
        function bse_cl_f() {
  // Get the checkbox
             ...
             <script>
                 ...
                 $(document).ready(function(){
                     $('.btn_update').click(function(){
                         let gst_in=$("#gst_in").val(); 
                         let exc_r_in=$("#exc_r_in").val();
                         let nsetm_in=$("#nsetm_in").val();
                         let nsetm_clr_in=$("#NSE_clr_no").val();
                         let bse_cl_in=$("#bse_cl_in").val();
                         let msei_in=$("#msei_in").val();
                        let msei_clr_in=$("#msei_clr_in").val();
                        let mcxtm_in=$("#mcxtm_in").val();
                        let mcx_clr_in=$("#mcx_clr_in").val();
                        let ncdex_in=$("#ncdex_in").val();
                        let ncdex_cl_in=$("#ncdex_cl_in").val();
                        let icex_in=$("#icex_in").val();
                        
                        let sebi_r_dp_in=$("#sebi_r_dp_in").val();
                        
                        let nsdl_in=$("#nsdl_in").val();
                        let cmbpid_in=$("#cmbpid_in").val();
                        let cdsl_in=$("#cdsl_in").val();
                        let sebi_anyl_in=$("#sebi_anyl_in").val();
                        let sebi_pms_in=$("#sebi_pms_in").val();
                         $.ajax({
                             //clr_no_in:clr_no_in,
                             url:'update_save',
                             method:"POST",
                             data:{
                                exc_r_in:exc_r_in,gst_in:gst_in,nsetm_in:nsetm_in,bse_cl_in:bse_cl_in,msei_in:msei_in,
                             msei_clr_in:msei_clr_in,mcxtm_in:mcxtm_in,ncdex_in:ncdex_in,ncdex_cl_in:ncdex_cl_in,
                             icex_in:icex_in,sebi_r_dp_in:sebi_r_dp_in,
                             nsdl_in:nsdl_in,cdsl_in:cdsl_in,sebi_anyl_in:sebi_anyl_in,sebi_pms_in:sebi_pms_in,cmbpid_in:cmbpid_in,
                             mcx_clr_in:mcx_clr_in,nsetm_clr_in:nsetm_clr_in
